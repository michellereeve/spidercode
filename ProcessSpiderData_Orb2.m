...
if ~exist([kPathname filePrefix '.mat'],'file')
    ...
    while threshGood == 'N'
        ...
        threshans = inputdlg('Is Vel Theshold OK? (Y/N)','VelThresh Status',1,{'N'});
...
% [~] = SaveFigAsPDF(f6,savePathN,filePrefix,'_LegLengths');
% close(f6);
...
% [~] = SaveFigAsPDF(f7,savePathN,filePrefix,'_LegAngles');
% close(f7);
...
%     [~] = SaveFigAsPDF(f11,savePathN,filePrefix,'_HilbertPhase');
%     close(f11);
