function [] = ProcessOrbSpiderDataR4ablation()
% A script to process spider gait data ( orb-weaver, R4ablation trials)
...
% column numbers (for R4ablation data):
% 1 = frame number
% 2 = time (sec)
% 3-4 = bodyCOM X,Y
% 5-6 = bodyBack X,Y
...
while filtGood == 'N'
    ...
    f1=figure;
    plot(time,rawTotVel(:,4),'r'); % col 4 - leg L1
    hold on;
...
% label legs
leg_labels = {'L1','L2','L3','L4','R1','R2','R3'};
...
% For each foot, detect foot contact periods
    ...
    gaitDiagramData = legContacts.* repmat([1 2 3 4 5 6 7],length(legContacts),1);
...
% velocities calculated by the SPLINE function. Works on Matrices with 10
% cols (1-3 = body, 4-10 = legs)
